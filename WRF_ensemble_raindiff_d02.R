rm(list=ls())
setwd('/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/out/')
...
for(day in 27:31)
  ...
  {
    dir1=paste("/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble/out/200705",day,hour,"/",sep="")
    dir2=paste("/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/out/200705",day,hour,"/",sep="")
...
pdf(file="/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/D02_totalESBrain_notopo_allruns.pdf",
    width=6,height=4)
...
write.csv(meanR,file="/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/out/D02_totalESBrain.csv")
...
dailyrain<-dailyrain_nt<-dailymaxrain<-dailymaxrain_nt<-array(0,c(325,200,30,20))
...
save(dailyrain,dailymaxrain,dailyrain_nt,dailymaxrain_nt,file="/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/WRFdailyrain_ensemble_d02.RData")

load('WRFdailyrain_ensemble_d02.RData')
...
pdf(file="/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/D02_dailyrainthresh_notopo.pdf",
    width=6,height=4)
...
pdf(file="/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/D02_maxarea25mm_notopo.pdf",
    width=6,height=4)
plot(a,col=rgb(0,0,1,1/4),xlim=lims,ylim=range(0,a$y,b$y),
...
pdf(file="/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/D02_maxarea50mm_notopo.pdf",
    width=6,height=4)
plot(a,col=rgb(0,0,1,1/4),xlim=lims,ylim=range(0,a$y,b$y),
     xlab="Rain area (%)",ylab="Frequency",cex.main=1.2,
...
lims[1]=floor(lims[1]/5)*5
  ...
  pdf(file="/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/D02_maxarea100mm_notopo.pdf",
      width=6,height=4)
  plot(a,col=rgb(0,0,1,1/4),xlim=lims,ylim=range(0,a$y,b$y),
       xlab="Rain area (%)",ylab="Frequency",cex.main=1.2,
...
pdf(file=paste("/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/D02_Jun",day,"_area25mm_notopo.pdf",sep=""),
    width=6,height=4)
plot(a,col=rgb(0,0,1,1/4),xlim=lims,ylim=range(0,a$y,b$y),
     xlab="Rain area (%)",ylab="Frequency",cex.main=1.2,
     main=paste("Percentage area above 25 mm"))
...
for(i in 1:30)
  for(n in 1:20)
  {
    meanR[i,n]=mean(dailyrain[,,i,n]*mask10a,na.rm=T)
    meanR_nt[i,n]=mean(dailyrain_nt[,,i,n]*mask10a,na.rm=T)
  }
...
pdf(file="/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/D02_dailyrainsig_notopo.pdf",
    width=6,height=4)
...
## How does the mean & certainty vary between 5, 10 & 20-member subsets

library(boot)

means<-means2<-matrix(0,10000,3)
...
for(i in 1:10000)
{
  ...
  a=sample(meanR[,3],10)
  means[i,2]=mean(a)
  a=sample(meanR[,3],replace=T)
  means[i,3]=mean(a)
  
  a=sample(meanR[,3],5,replace=T)
  means2[i,1]=mean(a)
  a=sample(meanR[,3],10,replace=T)
  means2[i,2]=mean(a)
  a=sample(meanR[,3],replace=T)
  means2[i,3]=mean(a)
}
...
pdf(file="/srv/ccrc/data36/z3478332/WRF/output/ERAI_R1_ensemble_notopo/D02_totalESBraindiff_ensemble.pdf",
    width=6,height=4)
plot(a,col="red",xlim=lims,ylim=range(0,a$y,b$y,c$y),
     xlab="Rain area (%)",ylab="Frequency",cex.main=1.2,
     main=paste("Mean ESB rainfall difference, 10000-member bootstrap"),lwd=4)
lines(b,col="blue",lwd=4)
lines(c,col="black",lwd=4)
#legend("topleft",legend=c("20 sample bootstrap","10 samples, no replacement","5 samples, no replacement"),
#       col=c("black","blue","red"),lwd=4,cex=1,bty="n")  
a=density(means2[,1])
b=density(means2[,2])
lines(b,col="blue",lwd=4,lty=2)
lines(a,col="red",lwd=4,lty=2)
legend("topright",legend=c("20 sample bootstrap","10 samples, no replacement","5 samples, no replacement","10 samples, with replacement","5 samples, with replacement"),
       col=c("black","blue","red","blue","red"),lwd=4,bty="n",lty=c(1,1,1,2,2))  
dev.off()
