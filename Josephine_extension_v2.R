...
######### Write out each ensemble member in RHtest format

rm(list=ls())
setwd("/srv/ccrc/data34/z3478332/20CR/ECLs/")
load("20CR_ECLdates_v3.RData")
source('~/Documents/R/RHtestsV4.r')
tmp=read.table("20CR_annfreq_meanofens.txt")

for(i in 1:56)
{
  tmp[,4]=annfreq[,i]
  write.table(tmp,file=paste("20CR_annfreq_",i,".txt",sep=""),row.names=F,col.names=F,sep="\t")
  FindU(paste("20CR_annfreq_",i,".txt",sep=""),paste("output/RHtests/20CR_annfreq_",i,sep=""),"-999")
  
  tmp2=scan(paste("output/RHtests/20CR_annfreq_",i,"_mCs.txt",sep=""),nmax=1)
  
  if(tmp2>0)
  {
  tmp2=read.table(paste("output/RHtests/20CR_annfreq_",i,"_mCs.txt",sep=""),skip=1,stringsAsFactors=F)
  tmp3=data.frame(Member=rep(i,length(tmp2[,1])),Year=tmp2[,3]/10000,Sig=tmp2[,2],stringsAsFactors=F)
  if(i==1) Breaks=tmp3 else Breaks=rbind(Breaks,tmp3)
  }
}

length(unique(Breaks$Member))

years=seq(1871,2012)
breaks2=cbind(years,matrix(0,length(years),2))
for(i in 1:length(years)) 
  {
  breaks2[i,2]=length(which(Breaks$Year==years[i]))
  breaks2[i,3]=length(which(Breaks$Year==years[i] & Breaks$Sig=="Yes"))
}
colnames(breaks2)=c("Year","Breaks","Significant Breaks")
barplot(breaks2[,3])
