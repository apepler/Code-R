...
for(y in 1:33)
  for(m in 1:12)
  {
    ...
    Tasman[y,m,1]=mean(sst[J,I,n],na.rm=T)
    n=n+1
  }
...
for(y in 1:33)
  for(m in 1:12)
  {
    ...
    Tasman[y,m,3]=mean(sst1[J,I,n]*mask2[J,I],na.rm=T)
    n=n+1
  }
...
for(y in 1:33)
  for(m in 1:12)
  {
    I=which(lat1>=-41 & lat1<=-24)
    J=which(lon1>=148 & lon1<=161)
    Tasman[y,m,2]=mean(sst1[J,I,n],na.rm=T)
    n=n+1
  }

library(abind)
Tasman2=abind(apply(Tasman[,,],c(1,3),mean),apply(Tasman[,3:5,],c(1,3),mean),apply(Tasman[,6:8,],c(1,3),mean),
              apply(Tasman[,9:11,],c(1,3),mean),apply(Tasman[,c(12,1:2),],c(1,3),mean),
              apply(Tasman[,5:10,],c(1,3),mean),apply(Tasman[,5:10,],c(1,3),mean),along=3)
dimnames(Tasman2)[[3]]=c("Annual","Autumn","Winter","Spring","Summer","Cool","Warm")

##Now, fix summer & warm
Tasman2[1:32,,5]=apply(abind(Tasman[1:32,12,],Tasman[2:33,1:2,],along=2),c(1,3),mean)
Tasman2[1:32,,7]=apply(abind(Tasman[1:32,11:12,],Tasman[2:33,1:4,],along=2),c(1,3),mean)
...
corrM=array(0,c(3,7))
rownames(corrM)=names
colnames(corrM)=c("Annual","Autumn","Winter","Spring","Summer","Cool","Warm")
  for(j in 1:3)
    for(k in 1:7)
     corrM[j,k]=cor(ECLs[,2,k],Tasman2[,j,k],use="pairwise.complete.obs")
