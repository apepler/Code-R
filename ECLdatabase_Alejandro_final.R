...
indir="/srv/ccrc/data25/z3444417/ECLs/Browning/5_WIND_PRECIP/v14p1_in-v14p1_out/1990-2010/PG2/WRF-N50/txt/"
outdir="/srv/ccrc/data34/z3478332/CMIP-WRF-ECLs/Alejandro/final/"
...
for(j in 4)
  for(l in 1:2)
  {
    infile=paste(indir,names2[j],"-global-present-",res[l],"-6_v14p1-D2-PG2.csv",sep="")        
    outfile=paste("Alejandro_",names1[j],"_res",res[l],"_9009_pg0.7_final",sep="")
    ECLdatabase(infile,outfile,0.7)
  }

for(j in 1:3)
  for(l in 1:2)
  {
    infile=paste(indir,"WRF-50-3-nnrp-",wrfv[j],"-present-",res[l],"-6_v14p1-D2-PG2.csv",sep="")        
    outfile=paste("Alejandro_ncep_wrf",wrfv[j],"_res",res[l],"_9009_pg",thresh[l],"_final",sep="")
    ECLdatabase(infile,outfile,thresh[l])
  }


for(k in 1:2)
{
  if(k==1) indir="/srv/ccrc/data25/z3444417/ECLs/Browning/5_WIND_PRECIP/v14p1_in-v14p1_out/1990-2010/PG2/WRF-N50/txt/" else
    indir="/srv/ccrc/data25/z3444417/ECLs/Browning/5_WIND_PRECIP/v14p1_in-v14p1_out/2060-2080/PG2/WRF-N50/txt/"
  
  for(i in 1:4)
    for(j in 1:3)
        for(l in 1:2)
        {
          infile=paste(indir,"WRF-50-3-",cmip[i],"-",wrfv[j],"-",period[k],"-",res[l],"-6_v14p1-D2-PG2.csv",sep="")        
          outfile=paste("Alejandro_",cmipUM[i],"_wrf",wrfv[j],"_res",res[l],"_",periodUM[k],"_pg",thresh[l],"_final",sep="")
          ECLdatabase(infile,outfile,thresh[l])
        }
}
...
ECLdatabase<-function(infile,outfile,thresh=NA)
{
  ...
  if(length(data$DEPTH_MEAN)==length(data[,1]))
    data2=data.frame(ID=data[,1],Fix=rep(0,length(data[,1])),Date=data$YEAR*10000+data$MONTH*100+data$DAY,Time=data$HOUR,
                   Lon=data$LON_CEN,Lat=data$LAT_CEN,Location=rep(0,length(data[,1])),MSLP=data$P_CEN,PG200=data$PG200,
                   PG500=data$PG500,Rad=as.numeric(data$SIZE_MEAN),RadR=as.numeric(data$SIZE_MEAN_RUD),LAP=as.numeric(data$LAP_MEAN),Depth=as.numeric(data$DEPTH_MEAN),
                   MeanR=as.numeric(data$PR_MEAN_500),MaxR=as.numeric(data$PR_MAX_500),MeanW=as.numeric(data$WS10_MEAN_500),MaxW=as.numeric(data$WS10_MAX_500)) else
                     data2=data.frame(ID=data[,1],Fix=rep(0,length(data[,1])),Date=data$YEAR*10000+data$MONTH*100+data$DAY,Time=data$HOUR,
                                      Lon=data$LON_CEN,Lat=data$LAT_CEN,Location=rep(0,length(data[,1])),MSLP=data$P_CEN,PG200=data$PG200,
                                      PG500=data$PG500,MeanR=as.numeric(data$PR_MEAN_500),MaxR=as.numeric(data$PR_MAX_500),MeanW=as.numeric(data$WS10_MEAN_500),MaxW=as.numeric(data$WS10_MAX_500))
  ...
  events<-data.frame(ID=x$values,Length=x$lengths,Date1=rep(0,length(x$values)),Date2=rep(0,length(x$values)),
                     MSLP=rep(0,length(x$values)),PG200_max=rep(0,length(x$values)),PG500_max=rep(0,length(x$values)),
                     LAP_max=rep(NaN,length(x$values)),Rad_mean=rep(NaN,length(x$values)),Loc=rep(0,length(x$values)))
  ...
  for(i in 1:length(events[,1])) 
  {
    ...
    events$MSLP[i]=min(fixes$MSLP[I],na.rm=T) 
    events$PG200_max[i]=max(fixes$PG200[I],na.rm=T) 
    events$PG500_max[i]=max(fixes$PG500[I],na.rm=T) 
    events$LAP_max[i]=max(fixes$LAP[I],na.rm=T) 
    events$Rad_mean[i]=mean(fixes$Rad[I],na.rm=T) 
    if(sum(fixes$Location[I])>0) events$Loc[i]=1 ##Ever in ECL region
  }
  
  ## Take only the events where at least one fix is closed & in the ECL region
  ##And duration is at least 2 consecutive fixes
  a=dim(events)
  I<-which(events$Loc>0)
  events2<-events[I,1:(a[2]-1)]
  ...
  events2$PG2<-events2$MSLP2<-events2$Length2<-0
  events2$MaxPointWind<-events2$MaxMeanWind<-events2$MaxPointRain<-events2$MaxMeanRain<-events2$TotalRain<-0
  ...
  for(i in 1:length(events2[,1]))
  {
    ...
    events2$MaxMeanRain[i]=max(fixes2$MeanR[I],na.rm=T)
    events2$MaxPointRain[i]=max(fixes2$MaxR[I],na.rm=T)
    events2$TotalRain[i]=sum(fixes2$MeanR[I],na.rm=T) 
    events2$MaxMeanWind[i]=max(fixes2$MeanW[I],na.rm=T) 
    events2$MaxPointWind[i]=max(fixes2$MaxW[I],na.rm=T) 
  }
